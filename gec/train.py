...
def main(args):
    ...
    train(databin_path, model_config, ckpt_dir, restore_ckpt)


def train(databin_path, model_config, ckpt_dir, restore_ckpt):
    os.makedirs(ckpt_dir, exist_ok=True)
    logging.info(f"[Train] working on {ckpt_dir}")

    prompt = f"python -m torch.distributed.launch --nproc_per_node 8 $(which fairseq-train) {databin_path} " \
             f"{model_config} " \
             f"--save-dir {ckpt_dir} "
